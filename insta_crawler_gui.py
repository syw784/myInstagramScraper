...
class insta_crawler_Ui(QtWidgets.QMainWindow):
    ...
    def get_downloadable_url_from_node(self, node):
        ...
        if node['is_video']:
            try:
                ...
                self.text_log.append('video needs to be downloaded from the subpage')
        else:
            try:
    ...
    def download_first_page(self, link, end_cursor = False):
        ...
        self.text_log.append('crawling ' + link)
        dl_link = {}
        if end_cursor:
        ...
        if self.download_from_dic(dl_link, parsed_path) and not param["ignore_downloaded_and_loop"]:
            self.text_log.append('encountered duplicates; to continue, enable ignore_downloaded_and_loop')
            return
        self.download_from_query(ins_id, end_cursor, parsed_path, link)
    ...
    def download_from_query(self, ins_id, end_cursor, parsed_path, link):
        while end_cursor:
            ...
            self.text_log.append('next query:')
            self.text_log.append(link)
            self.text_log.append("'" + end_cursor + "'")
            self.text_log.append(next_query)
            ...
            if self.download_from_dic(dl_link, parsed_path) and not param["ignore_downloaded_and_loop"]:
                self.text_log.append('encountered duplicates; to continue, enable ignore_downloaded_and_loop')
                return
        self.text_log.append('completed crawling ' + link)
    ...
    def login(self, brawser):
        ...
        time.sleep(1)
        try:
            brawser.find_element_by_xpath('//*[@id="react-root"]/section/main/div/article/div/div[1]/div/form/div[2]/div/label/input').send_keys(usr_name)
            brawser.find_element_by_xpath('//*[@id="react-root"]/section/main/div/article/div/div[1]/div/form/div[3]/div/label/input').send_keys(pswd)
            time.sleep(1)
            brawser.find_element_by_xpath('//*[@id="react-root"]/section/main/div/article/div/div[1]/div/form/div[4]/button/div').click()
            self.text_log.append("loggged in with usr name " + usr_name + '...')
            time.sleep(5)
            return True
        except:
            self.text_log.append("try again:(")
            return False

    def crawling(self):
        if len(self.param["dl_from_end_cursor"]) > 0:
            ...
            for i in self.param["ins_model"]:
                try:
                    ...
                    self.text_log.append("failed crawling " + i)
        self.text_log.append('jobs finished')


    def click_scrawl(self):
        if not self.logged_in:
            self.logged_in = self.login(self.driver)
        if not self.logged_in: return False
        self.load_param()
        # for i in self.param['ins_model']:
        #     self.text_log.append(i)
        t = threading.Thread(target = self.crawling)
        t.daemon = True
        t.start()
